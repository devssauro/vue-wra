  ...
  <v-main fill-height>
    <v-container>
              ...
              <template v-for="matchup in matchups">
                <v-expansion-panel :key="matchup.id">
                  <v-expansion-panel-header>
                    <span
                      >{{ matchup.datetime }} | {{ matchup.team1.name }} X
                      {{ matchup.team2.name }}</span
                    >
                    <v-spacer /> <span />
                  </v-expansion-panel-header>
                  <v-expansion-panel-content>
                    <v-row>
                      <template
                        v-if="
                          matchup.maps.length === 1 ||
                            matchup.maps.length === 2 ||
                            matchup.maps.length === 3 ||
                            matchup.maps.length === 4 ||
                            matchup.maps.length === 6
                        "
                      >
                      ...
                      <v-col cols="12">
                        <v-btn
                          ...
                          depressed
                          color="success"
                          v-if="isOperationalUser"
                          @click="newMap(matchup.id)"
                        >
                          Adicionar mapa
                        </v-btn>
                      </v-col>
      ...
      <v-dialog max-width="650" v-model="matchupDialog" persistent>
        <matchup-dialog
          :tournament-id="selectedTournaments"
          v-if="matchupDialog"
          @saved="closeMatchupDialog"
        />
      </v-dialog>
      <v-dialog fullscreen v-model="mapDialog">
        <map-dialog
          v-if="mapDialog"
          @saved="closeMapDialog"
