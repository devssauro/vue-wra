                  ...
                  <v-container>
                    <v-row>
                      <v-col class="mb-n8" cols="12">
                        <v-text-field
                          v-model="selectedTournament.name"
                          outlined
                          label="Nome"
                        />
                      </v-col>
                      <v-col class="mb-n8" cols="12">
                        <v-text-field
                          v-model="selectedTournament.tag"
                          outlined
                          label="Tag"
                        />
                      </v-col>
                      <v-col class="mb-n8" cols="12">
                        <v-text-field
                          v-model="selectedTournament.region"
                          outlined
                          label="Região"
                        />
                      </v-col>
                      <v-col class="mb-n8" cols="12">
                        <v-text-field
                          v-model="selectedTournament.split"
                          outlined
                          label="Split"
                        />
                      </v-col>
                      <v-col class="mb-n8" cols="12">
                        <v-text-field
                          v-model="selectedTournament.phases"
                          outlined
                          label="Fases do campeonato (separados por vírgula)"
                        />
                      </v-col>
                      <v-col class="mb-n8" cols="12">
                        <v-checkbox
                          v-model="selectedTournament.female_only"
                          label="Campeonato feminino"
                        />
                      </v-col>
                    </v-row>
                  </v-container>
                ...
                <v-tab-item>
                  <v-container fill-height>
                    <v-row>
                      ...
                      <v-col v-if="editLineup" cols="12" class="mb-n8">
                        <v-text-field
                          v-model="currentLineup.entry_phase"
                          outlined
                          dense
                          label="Fases de entrada"
                        />
                      </v-col>
                      ...
                      <v-col>
                        <v-list>
                          <template
                            v-for="(item, index) in selectedTournament.lineups"
                          >
  ...
  methods: {
    ...
    saveLineup() {
      if (this.currentLineupIndex === null)
        this.selectedTournament.lineups.push(
          Object.assign({}, this.currentLineup)
        );
      else
        this.selectedTournament.lineups[
    ...
    saveDialog() {
      console.log(this.selectedTournament.id);
      const method =
        this.selectedTournament.id === null ||
        this.selectedTournament.id === undefined
          ? axios.post
          : axios.put;
      const url =
        this.selectedTournament.id === null
          ? "/v1/tournament"
          : `/v1/tournament/${this.selectedTournament.id}`;
      method(url, this.selectedTournament).then((res) => {
        this.getTournaments();
      });
      this.closeDialog();
    },
