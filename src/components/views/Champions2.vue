            ...
            <v-row>
              <v-col cols="12" sm="12" md="5">
                <v-card
                  ...
                  outlined
                  v-if="
                    search.champion_id !== null &&
                      search.champion_id !== undefined
                  "
                >
                  <v-card-title>
                    <v-container>
                      <v-row align="start">
                        ...
                        <v-col cols="8" class="py-0">
                          ...
                          <p class="headline">
                            {{ avg_stats.avg_kills }} /
                            {{ avg_stats.avg_deaths }} /
                            {{ avg_stats.avg_assists }}
                          </p>
                          <p class="mb-0">
                            <span> {{ avg_stats.ddpm }} DDPM </span>
                            <span class="pl-4"> {{ avg_stats.gpm }} GPM </span>
                          </p>
                        </v-col>
                      </v-row>
                    </v-container>
            ...
            <v-tab-item>
              ...
              >
                ...
                <template v-slot:[`item.avg_kills`]="{ item }">
                  ...
                  <span
                    class="green--text"
                    v-if="item.avg_kills > avg_stats.avg_kills"
                  >
                ...
                <template v-slot:i[`tem.avg_deaths`]="{ item }">
                  ...
                  <span
                    class="red--text"
                    v-if="item.avg_deaths > avg_stats.avg_deaths"
                  >
                ...
                <template v-slot:[`item.avg_assists`]="{ item }">
                  ...
                  <span
                    class="green--text"
                    v-if="item.avg_assists > avg_stats.avg_assists"
                  >
                    (+{{
                ...
                <template v-slot:[`item.avg_kda`]="{ item }">
                  ...
                  <span
                    class="green--text"
                    v-if="item.avg_kda > avg_stats.avg_kda"
                  >
                ...
                <template v-slot:[`item.agt`]="{ item }">
                  ...
                  <span
                    class="green--text"
                    v-if="item.diff_agt.indexOf('-') > -1"
                  >
                    ({{ item.diff_agt }})
                  </span>
                  <span class="red--text" v-else> ({{ item.diff_agt }}) </span>
                </template>
                <template v-slot:[`item.agt_win`]="{ item }">
                  ...
                  <span v-if="item.agt_win !== '-'">
                    <span
                      class="green--text"
                      v-if="item.diff_agt_win.indexOf('-') > -1"
                    >
                      ({{ item.diff_agt_win }})
                    </span>
                    <span class="red--text" v-else>
                ...
                <template v-slot:[`item.agt_loss`]="{ item }">
                  ...
                  <span v-if="item.agt_loss !== '-'">
                    <span
                      class="green--text"
                      v-if="item.diff_agt_loss.indexOf('-') == -1"
                    >
                      ({{ item.diff_agt_loss }})
                    </span>
                    <span class="red--text" v-else>
...
export default {
  ...
  computed: {
    axiosParams() {
      ...
      if (this.search.t !== null) params.append("t", this.search.t);
      if (this.search.patch !== null) params.append("patch", this.search.patch);
      if (this.search.side !== null) params.append("side", this.search.side);
  ...
  methods: {
    ...
    championChanged() {
      this.selectedChampion = this.champions.filter(
        (c) => c.id === this.search.champion_id
      )[0];
      this.availableRoles = this.roles.filter(
        (r) => this.selectedChampion.roles.indexOf(r.tag) > -1
      );
    ...
    getChampions() {
      axios.get(`v1/view/champion`, { params: this.search }).then((res) => {
        ...
        const _champions = this.champions.filter(
          (c) => c.id === this.selectedChampion.id
        );
    ...
    getChampionsFromRole() {
      axios
        .get(`v1/view/champion/role/${this.search.role}`, {
    ...
    getSideStats() {
      axios
        .get(`v1/view/champion/${this.selectedChampion.id}/side`, {
          params: this.axiosParams,
        })
        .then((res) => {
    ...
    getAllMatches() {
      axios
        .get(`v1/view/champion/${this.selectedChampion.id}/all_matches`, {
          params: this.axiosParams,
        })
        .then((res) => {
    ...
    getPlayers() {
      axios
        .get(`v1/view/champion/${this.selectedChampion.id}/players`, {
          params: this.axiosParams,
        })
        .then((res) => {
    ...
    getPatches() {
      axios.get(`v1/patch`, { params: this.axiosParams }).then((res) => {
        ...
        if (_patch.length === 0) this.search.patch = null;
